...
public class SelectEventUi extends AbstractUi {
	...
	public void show() {
		...
		if (start.compareTo(end) > 0) {
			System.out.println("검색종료일은 검색개시일 이후의 날짜를 입력해주세요.");
			show();
			return;
		}
		// 이벤트 취득
	...
	protected Date getDate() {
		...
		if (new Date().after(date)) {
			System.out.println("현재보다 과거인 날짜는 입력할 수 없습니다. 다시 날짜를 입력해 주세요.");
			return getDate();
		}
	...
	protected void showMenu(String wanted) {
		System.out.printf("%s를 입력하고, Enter를 눌러주세요.%n", wanted);
		System.out.println("아무것도 입력하지 않고 Enter를 누르면 메뉴로 이동합니다.");
		System.out.println(DATE_FORMAT_MESSAGE);
	}
}
